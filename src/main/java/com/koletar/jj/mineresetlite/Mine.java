...
public class Mine implements ConfigurationSerializable {
    ...
    public Mine(World world, Location teleportLocation, String name, int minX, int minY, int minZ, int maxX, int maxY, int maxZ) {
        this(minX, minY, minZ, maxX, maxY, maxZ, name, world);
    ...
    public Mine(Map<String, Object> me) {
        try {
            ...
            Object object = me.get("teleportLocation");
            if(object != null) {
                teleportLocation = StringTools.locFrom((String) object);
            }
        } catch (Throwable t) {
    ...
    public Map<String, Object> serialize() {
        ...
        me.put("teleportLocation", StringTools.locTo(teleportLocation));
    ...
    public void reset() {
        ...
        for (Player p : Bukkit.getServer().getOnlinePlayers()) {
            Location l = this.getTeleportLocation();
            if(l == null) {
                l = p.getLocation();
                l = new Location(world, l.getX(), maxY + 2D, l.getZ());
            }
            if (isInside(p)) {
                p.teleport(l);
            }
